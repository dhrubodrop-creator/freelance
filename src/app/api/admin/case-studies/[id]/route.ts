...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const { error } = await supabaseAdmin().from("case_studies").update(parsed.data).eq("id", params.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  await logAdminAction(admin.id, "update", "case_study", params.id, parsed.data);
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  await logAdminAction(admin.id, "delete", "case_study", params.id);
