  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-xl border border-success/30 bg-success/5 px-6 py-10 text-center">
        ...
        <p className="font-heading text-h4 font-semibold">Message sent</p>
        <p className="max-w-sm text-sm text-muted-foreground">
          Thanks for reaching out — we&rsquo;ll get back to you by email shortly.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {errors.message && <p className="text-micro text-destructive">{errors.message.message}</p>}
      </div>
