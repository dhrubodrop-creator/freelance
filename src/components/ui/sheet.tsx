...
function SheetOverlay({ className, ...props }: React.ComponentProps<typeof SheetPrimitive.Overlay>) {
  return (
    <SheetPrimitive.Overlay
      ...
      className={cn("fixed inset-0 z-50 bg-ink-950/40 backdrop-blur-sm", className)}
      {...props}
    />
  );
}
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          "fixed z-50 flex flex-col gap-4 border-border bg-popover p-6 text-popover-foreground shadow-lifted",
          sideClasses[side],
          className
        )}
        {...props}
